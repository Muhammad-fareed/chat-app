...
Echo.private('users.{{ Auth()->User()->id }}')
 ...
 "
    ...
    <div class="border-b flex flex-col overflow-y-scroll-hidden grow h-full">
        {{-- header --}}
        <header class="sticky w-full inset-x-0 flex pb-[5px] pt-[5px] z-10 top-0 bg-white border-b">
            <div class="flex w-full items-center px-2 lg-px-4 gap-2 md:gap-5">
                <a href="" class="shring-0 lg:hidden">
     ...
     "
            ...
            @if ($loadMessages)
                ...
                @foreach ($loadMessages as $key => $message)
                    @if ($key > 0)
                        @php
                            $prevMessages = $loadMessages->get($key - 1);
                        @endphp
                    @endif
                    <div
                    wire:key="{{time().$key}}"
                    @class([
                        ...
                        <div @class([
                            'shrink-0',
                            'invisible' => $prevMessages?->sender_id == $message?->sender_id,
                        ...
                        {{-- body --}}
                        <div @class([
                            'flex flex-wrap text-[15px] rounded-xl px-2.5 flex-col text-black bg-[#f6f6f8]',
                            ...
                            <div class="ml-auto flex gap-2">
                                ...
                                {{--  Message status ,only show if auth --}}
        ...
        <footer class="shrink-0 z-10 bg-white inset-x-0">
            <div class="p-2 border-t">
                <form x-data="{ body: @entangle('body').defer }" @submit.prevent="$wire.sendMessage(); body=''" method="POST"
                    autocapitalize="off">
                    @csrf
                    ...
                    <div class="grid grid-cols-12">
                        ...
                        <button :disabled="!body.trim()" class="col-span-2" type='submit'>Send</button>
                    </div>
                </form>
                @error('body')
                    <p>{{ $message }}</p>
