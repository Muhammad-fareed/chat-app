...
x-init="$nextTick(() => {
    ...
    <header class="px-3 z-10 bg-white sticky top-0 w-full py-2">
        ...
        <div class="flex gap-3 items-center overflow-x-scroll p-2 bg-white">
            <button x-on:click="type='all'" :class="{ 'bg-blue-100 border-0 text-black': type == 'all' }"
            ...
            <button x-on:click="type='deleted'" :class="{ 'bg-blue-100 border-0 text-black': type == 'deleted' }"
                class="inline-flex justify-center items-center rounded-full gap-x-1 text-xs font-medium px-3 lg:px-5 py-1 lg:py-2.5 border">
                Deleted
            </button>
        </div>
        ...
        <ul class="p-2 grid w-full spacey-y-2">
            @if ($conversations)
                @foreach ($conversations as $conversation)
                    <li wire:key="{{ $conversation->id }}" id="conversation-{{ $conversation->id }}"
                        ...
                        <a href="" class="shirnk-0">
                            <x-avatar src="https://picsum.photos/seed/picsum/500/500" />
                        </a>
                            ...
                            <a href="{{ route('chat', $conversation->id) }}"
                                ...
                                <div class="flex gap-x-2 items-center">
                                    ...
                                    @if ($conversation->unreadMessagesCount() > 0)
                                        <span
                                            class="font-bold p-px px-2 text-xs shrink-0 rounded-full bg-blue-500 text-white">{{ $conversation->unreadMessagesCount() }}</span>
                                    @endif
                                </div>
